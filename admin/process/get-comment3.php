...
if (isset($_REQUEST["customActionType"]) && $_REQUEST["customActionType"] == "group_action") {
  ...
  case '1':  
    $sqlaction="DELETE FROM `comment` WHERE `id` IN (";
    $i=1;
...
$sql="SELECT * FROM `comment` WHERE 1";
	if ($_REQUEST['comment_id'] != NULL)
	{
		$sql.=" AND `id` LIKE '%".$_REQUEST['comment_id']."%'";
	}
	if ($_REQUEST['comment_user'] != NULL)
	{
		$sql.=" AND `userid` LIKE '%".$_REQUEST['comment_user']."%'";
	}  
	if ($_REQUEST['comment_book'] != NULL)
	{
		$sql.=" AND `idb` LIKE '%".$_REQUEST['comment_book']."%'";
	}
	if ($_REQUEST['comment_content'] != NULL)
	{
		$sql.=" AND `content` LIKE '%".$_REQUEST['comment_content']."%'";
	}
	if ($_REQUEST['comment_time1'] != NULL)
	{
	$sql.=" AND `time` >= ".strtotime($_REQUEST['comment_time1'])."";
	}
	if ($_REQUEST['comment_time2'] != NULL)
	{
	$sql.=" AND `time` <= ".strtotime($_REQUEST['comment_time2'])."";
	}
...
while ($row2=@mysql_fetch_assoc($query2))
{
    ...
    $row2['content'],
    ti_me($row2['time']),
    '<a href="comment-edit.php?id='.$row2['id'].'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> Sửa</a>',
  );
}
$records["draw"] = $sEcho;
