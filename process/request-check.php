...
$title=$_POST['title'];
	...
	{	
			...
			if ($_FILES['image']['name'] != NULL)
			{
				if (in_array(getExt($_FILES['image']['name']),$mimes))
				{
					if ($_FILES["image"]["size"] <= 1024*1024*$size)
					{
						...
						$image="upload/".$name;
						$sqlr="insert into `request`(title,author,time,image,userid) values('".$title."','".$author."','".time()."','".$image."',".$_SESSION['userid'].")";
		...
		{
			$sqlr="insert into `request`(title,author,time,userid) values('".$title."','".$author."','".time()."',".$_SESSION['userid'].")";
			$queryr=@mysql_query($sqlr);
			$error['done']='1';
		}
	}	
